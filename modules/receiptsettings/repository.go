...
func (r *Repository) Get(ctx context.Context) (ReceiptSettings, error) {
	var shopName, shopAddr, shopPhone, logoPath, footer, tmpl *string

	err := r.db.QueryRow(ctx, `
		SELECT shop_name, shop_address, shop_phone, logo_path, footer, template
		FROM receipt_settings
		WHERE id = 1
	`).Scan(&shopName, &shopAddr, &shopPhone, &logoPath, &footer, &tmpl)
	if err != nil {
